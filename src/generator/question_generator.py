        ...
        for attempt in range(settings.MAX_RETRIES):
            try:
                ...
                if attempt == settings.MAX_RETRIES - 1:
                    raise CustomException(f"Generation failed after {settings.MAX_RETRIES} attemps : ", e)
    ...
    def generate_fill_blank(
        ...
        try:
            ...
            if "____" not in question.question:
                raise ValueError("Fill in blank should contain '____'")
            
            self.logger.info("Generated a valid Fill in blanks question")
            return question
    
        except Exception as e:
            self.logger.error(f"Failed to fillups : {str(e)}")
